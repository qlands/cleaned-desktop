...
QVariant cropInputsModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if (role == Qt::DisplayRole)
    {
        if (orientation == Qt::Horizontal)
        {
            ...
            if (section == 2) return "Urea (kg/ha)";
            if (section == 3) return "NPK (kg/ha)";
            if (section == 4) return "DAP (kg/ha)";
            if (section == 5) return "Ammonium nitrate (kg/ha)";
            if (section == 6) return "Ammonium sulfate (kg/ha)";
            if (section == 7) return "N solutions (kg/ha)";
            if (section == 8) return "Ammonia (kg/ha)";
        }
    }
    return QVariant();
}
...
QVariant cropInputsModel::data(const QModelIndex &index, int role) const
{
    ...
    {
        if (index.column() == 2) return items[index.row()].urea;
        if (index.column() == 3) return items[index.row()].npk;
        if (index.column() == 4) return items[index.row()].dap;
        if (index.column() == 5) return items[index.row()].ammonium_nitrate;
        if (index.column() == 6) return items[index.row()].ammonium_sulfate;
        if (index.column() == 7) return items[index.row()].n_solutions;
        if (index.column() == 8) return items[index.row()].ammonia;
    }
...
bool cropInputsModel::setData(const QModelIndex &index,const QVariant &value,int role)
{
    if (role == Qt::EditRole)
    {
        if (index.column() >= 2)
        {
            ...
            if (ok)
            {
                if (index.column() == 2) items[index.row()].urea = newValue;
                if (index.column() == 3) items[index.row()].npk = newValue;
                if (index.column() == 4) items[index.row()].dap = newValue;
                if (index.column() == 5) items[index.row()].ammonium_nitrate = newValue;
                if (index.column() == 6) items[index.row()].ammonium_sulfate = newValue;
                if (index.column() == 7) items[index.row()].n_solutions = newValue;
                if (index.column() == 8) items[index.row()].ammonia = newValue;
                emit modelChanged();
                return true;
            }            
        }
